...
- Supabase credentials: user creates a real Supabase project and fills real values into `.env.local` themselves (gitignored) — the agent never sees or handles the Project URL/publishable key directly; only variable names (`EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY`) go in the committed `.env.example`. Uses Supabase's modern publishable-key system (`sb_publishable_...`), the client-safe successor to the legacy anon/JWT key. Same self-managed-credential pattern as Sentry/PostHog.
...
- [x] `.env.example` (committed) + `.env.local` (gitignored) -- `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID`, `SENTRY_DSN`, `POSTHOG_API_KEY`, `POSTHOG_HOST` placeholders -- unblocks bootstrap before real credentials exist; nothing secret goes to git
